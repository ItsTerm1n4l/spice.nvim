local util = {}
local prime = require("prime.theme")
...
-- Only define prime if it's the active colorscheme
function util.onColorScheme()
	if vim.g.colors_name ~= "prime" then
		vim.cmd([[autocmd! prime]])
		vim.cmd([[augroup! prime]])
	end
end
...
util.contrast = function()
	vim.cmd([[augroup prime]])
	vim.cmd([[  autocmd!]])
	vim.cmd([[  autocmd ColorScheme * lua require("prime.util").onColorScheme()]])
...
function util.load()
	...
	vim.g.colors_name = "prime"

	-- load the most importaint parts of the theme
	local editor = prime.loadEditor()
	local syntax = prime.loadSyntax()
	local treesitter = prime.loadTreeSitter()
	local filetypes = prime.loadFiletypes()
	...
	local plugins = prime.loadPlugins()
	local lsp = prime.loadLSP()
	...
	if vim.g.prime_contrast == true then
		util.contrast()
	end
end
